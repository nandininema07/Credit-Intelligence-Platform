...
class PDFGenerator:
    ...
    async def generate_summary_report(self, summary_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            alert_stats = [
                ...
                ['Resolution Rate', f"{(resolved_alerts/total_alerts*100):.1f}%" if total_alerts > 0 else "0%"]
            ]
            
            table = Table(alert_stats)
            ...
            self.statistics['reports_generated'] += 1
            self.statistics['report_types']['summary'] = self.statistics['report_types'].get('summary', 0) + 1
    ...
    async def process_generation_queue(self):
        """Process PDF generation queue - compatibility method for AlertingEngine"""
        # This method is called by AlertingEngine but PDFGenerator doesn't have a queue system
        # For now, we'll just log that it was called
        logger.debug("PDF generation queue processing called - no queue system implemented")
        return True
